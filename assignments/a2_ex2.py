...
import assignments.a2_ex1 as ex1
...
def prepare_image(image: np.ndarray, 
    ...
    if (x < 0 or x + width > image[0].shape[1] or y < 0 or y + height > image[0].shape[0]):
        raise ValueError('Coordinate parameters are implausible \
                         (x or y is too small or too big) with x: {}, y: {}, width: {}, height: {} \
                         image height: {}, image-width: {}'.format(x, y, width, height, image[0].shape[1], image[0].shape[0]))
    ...
    mask[y:(y+pixelated_height), x:(x+pixelated_width)] = False
    ...
    #print(pixelated_section.shape)
    #print(pixelated_height, pixelated_width)
    y_end, x_end = img_copy[y:(y+pixelated_height), x:(x+pixelated_width)].shape
    img_copy[y:(y+pixelated_height), x:(x+pixelated_width)] = pixelated_section[:y_end, :x_end]
